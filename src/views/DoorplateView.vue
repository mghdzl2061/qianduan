  ...
  <div class="doorplate-page">
    ...
    <main v-loading="loading">
      <section v-if="house" class="plate-card">
        ...
        <div class="actions"><el-button type="primary" size="large" @click="goBusiness">{{ auth.token ? '进入业务办理' : '登录后办理业务' }}</el-button><el-button size="large" @click="$router.push('/notices')" v-if="auth.token">查看社区公告</el-button></div>
      </section>
      <section v-if="house" class="notice-section"><h2>社区公告</h2><article v-for="notice in house.notices" :key="notice.id"><span>{{ typeText[notice.type] }}</span><b>{{ notice.title }}</b><small>{{ formatDate(notice.publishedAt) }}</small></article></section>
    </main>
